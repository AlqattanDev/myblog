You are working on the GitHub-synced Astro blog project. The user has requested to refactor specific files tagged with @ symbols in their arguments: "$ARGUMENTS"
...
**Example parsing:**
- Input: "refactor @src/components/BlogPost.astro @src/utils/content.ts"
- Extract: ["src/components/BlogPost.astro", "src/utils/content.ts"]
...
- **Astro Component Analysis**: Component boundaries, slot usage, script/style organization
- **Blog Integration Mapping**: Content collection usage, GitHub API patterns, theme integration
- **Project Pattern Compliance**: Astro conventions, blog-specific patterns, file organization
- **Impact Assessment**: Astro config, content collections, build process that need updates
- **Import Update Analysis**: Components and utilities that import from the target file
- **Blog Stack Considerations**: Astro patterns, Tailwind usage, content processing
